...
class TokenService {
    ...
    verifyToken(token, callback) {
        jwt.verify(token, process.env.JWT_SECRET, [process.env.HASH_ALGORITHM || "HS256"], callback);
    }
    ...
    signToken(object, callback) {
        jwt.sign(
            ...
            { algorithm: process.env.HASH_ALGORITHM || "HS256" },
            callback
        );
    }

    signToken(object) {
        return jwt.sign(object, process.env.JWT_SECRET, { algorithm: process.env.HASH_ALGORITHM || "HS256"});
    }
