...
class SocketService {
    ...
    createChannel(channelName) {
        ...
        if(channel) {
            if(channel.initialized) success = true;
                ...
                channel.on(SOCKET.EVENT_CONNECTION, socket => { this[onConnection](channel, socket); });
                channel.on(SOCKET.EVENT_OPEN, socket => { this[onConnection](channel, socket); });
    ...
    notifyAddItemCollection(schema, item) {
        ...
        emitQueue.enqueue(this._io, SOCKET.EVENT_COLLECTION_ADD_ITEM, { schema: schema, item: item, createdAt: item.createdAt });
    }
    
    notifyRemoveItem(schema, item) {
        if (!this.isInitialized) return;
        emitQueue.enqueue(this._io, SOCKET.EVENT_COLLECTION_REMOVE_ITEM, { schema: schema, item: item });
    }
	
	notifyUpdateCollection(schema, items) {
        if (!this.isInitialized) return;
        emitQueue.enqueue(this._io, SOCKET.EVENT_UPDATE, { schema: schema, items: items });
    }

    notifyUpdateCollectionItem(schema, item) {
        if (!this.isInitialized) return;
        emitQueue.enqueue(this._io, SOCKET.EVENT_COLLECTION_UPDATE_ITEM, { schema: schema, item: item });
    }
    ...
    [getChannel](channelId) {
        var channel;
        var filtered = this._channels.filter((item) => {
    ...
    [onConnection](channel, socket) {
		...
		socket.once(SOCKET.COMMAND_SEND_BROADCAST, (data) => { channelRepo.sendBroadCast(data, channel, socket); });
        socket.once(SOCKET.COMMAND_REQUEST_AI_DATA, (data) => { channelRepo.requestAiData(data, socket); });
        socket.once(SOCKET.COMMAND_REQUEST_COLLECTION, (data) => { channelRepo.requestCollection(data, socket); });
        socket.once(SOCKET.COMMAND_SEND_PRIVATE_MESSAGE, (data) => { channelRepo.sendPrivateMessage(data, channel, socket); });
        socket.once(SOCKET.COMMAND_COLLECTION_ADD_ITEM, (data) => { channelRepo.addItem(data, channel, socket); });
        socket.once(SOCKET.COMMAND_COLLECTION_REMOVE_ITEM, (data) => { this._listener.removeItem(data, channel, socket); });
        socket.once(SOCKET.COMMAND_COLLECTION_UPDATE_ITEM, (data) => { this._listener.update(data, channel, socket); });
        socket.once(SOCKET.COMMAND_UPDATE, (data) => { this.channelRepo.update(data, channel, socket); });
        socket.once(SOCKET.COMMAND_USE_API, (data) => { this[onUseApi](data, channel, socket); });
        socket.once(SOCKET.COMMAND_DISCONNECT, (data) => { this[onDisconnect](data, channel, socket); });
        ...
        socket.once(SOCKET.EVENT_DISCONNECT, function(Socket, local) {
			...
			user.isOnline = false;
			collectRepo.updateItem("Client", user._id, {isOnline: false})
				.then(result => {
		...
		collectRepo.updateItem("Client", userObj._id, {connectedSince: userObj.connectedSince, isOnline: true})
			.then(result => {
